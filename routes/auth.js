...
router.post('/login', async function(req, res, next) {
  try {
    ...
    let user = await User.authenticate(username, password); //Add await to the call user.authenticate
   //Added the error handling in the route to raise 401 if username or password is incorrect
    if (!user) {
